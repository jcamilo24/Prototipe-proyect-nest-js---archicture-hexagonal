...
export class RedisIdempotencyService implements IdempotencyService {
  ...
  async handle<T>(
    ...
    if (!stored) {
      this.logger.log(`Idempotency cache miss | correlationId=${getCorrelationId() ?? '-'} key=${key} -> executing use case`);
      ...
      this.logger.log(`Idempotency stored | correlationId=${getCorrelationId() ?? '-'} key=${key} ttl=${this.ttlSeconds}s`);
      return response;
    }
    ...
    if (record.request_hash !== requestHash) {
      this.logger.warn(
        `Idempotency conflict | correlationId=${getCorrelationId() ?? '-'} key=${key} (same key, different payload)`,
      );
    ...
    this.logger.log(`Idempotency cache hit | correlationId=${getCorrelationId() ?? '-'} key=${key} -> returning cached`);
